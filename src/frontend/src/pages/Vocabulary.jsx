        ...
        {showModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <motion.div 
              ...
              className="bg-white/90 backdrop-blur-2xl rounded-[2.5rem] p-8 max-w-xl w-full shadow-2xl shadow-indigo-900/20 border border-white/50 relative max-h-[90vh] overflow-y-auto"
            >
              <div className="absolute top-0 right-0 w-64 h-64 bg-indigo-500/10 blur-[80px] rounded-full pointer-events-none"></div>
              
              <button onClick={() => setShowModal(false)} className="absolute top-6 right-6 p-2 text-slate-400 hover:text-slate-800 bg-slate-100 hover:bg-slate-200 rounded-full transition-colors z-10">
                <X className="w-5 h-5" />
              </button>
              
              <div className="mb-8 text-center relative z-10">
                <div className="w-16 h-16 bg-gradient-to-br from-indigo-100 to-purple-100 rounded-2xl flex items-center justify-center mx-auto mb-4 shadow-inner">
                  <Plus className="w-8 h-8 text-indigo-600" />
                </div>
                <h3 className="text-3xl font-black text-slate-800">Thêm Từ Mới</h3>
                <p className="text-slate-500 font-medium mt-2">Mở rộng vốn từ vựng của bạn bằng cách thêm thẻ thủ công hoặc tải lên file CSV.</p>
              </div>
              
              <div className="space-y-8 relative z-10">
                <form onSubmit={handleManualSubmit} className="space-y-5 bg-white/50 p-6 rounded-3xl border border-white/60 shadow-sm">
                  <div className="grid grid-cols-2 gap-5">
                    <div className="space-y-2">
                      <label className="text-xs font-black text-slate-500 uppercase tracking-widest pl-1">Hán tự</label>
                      <input required value={formData.hanzi} onChange={e => setFormData({...formData, hanzi: e.target.value})} className="w-full bg-white border border-slate-200 p-4 rounded-2xl focus:ring-4 focus:ring-indigo-100 focus:border-indigo-400 outline-none transition-all text-xl font-bold text-slate-800 text-center placeholder:text-slate-300 placeholder:font-normal" placeholder="你好" />
                    </div>
                    <div className="space-y-2">
                      <label className="text-xs font-black text-slate-500 uppercase tracking-widest pl-1">Pinyin</label>
                      <input required value={formData.pinyin} onChange={e => setFormData({...formData, pinyin: e.target.value})} className="w-full bg-white border border-slate-200 p-4 rounded-2xl focus:ring-4 focus:ring-indigo-100 focus:border-indigo-400 outline-none transition-all text-xl font-medium text-slate-600 text-center placeholder:text-slate-300" placeholder="nǐ hǎo" />
                    </div>
                  </div>
                  <div className="space-y-2">
                    <label className="text-xs font-black text-slate-500 uppercase tracking-widest pl-1">Nghĩa tiếng Việt</label>
                    <input required value={formData.meaning} onChange={e => setFormData({...formData, meaning: e.target.value})} className="w-full bg-white border border-slate-200 p-4 rounded-2xl focus:ring-4 focus:ring-indigo-100 focus:border-indigo-400 outline-none transition-all text-lg font-medium text-slate-700 placeholder:text-slate-300" placeholder="Xin chào" />
                  </div>
                  <div className="grid grid-cols-2 gap-5">
                    <div className="space-y-2">
                      <label className="text-xs font-black text-slate-500 uppercase tracking-widest pl-1">Cấp độ HSK</label>
                      <select value={formData.hsk_level} onChange={e => setFormData({...formData, hsk_level: e.target.value})} className="w-full bg-white border border-slate-200 p-4 rounded-2xl focus:ring-4 focus:ring-indigo-100 focus:border-indigo-400 outline-none transition-all font-bold text-slate-700 appearance-none cursor-pointer">
                        {[1,2,3,4,5,6].map(l => <option key={l} value={l}>HSK {l}</option>)}
                      </select>
                    </div>
                    <div className="space-y-2">
                      <label className="text-xs font-black text-slate-500 uppercase tracking-widest pl-1">Ví dụ</label>
                      <input value={formData.examples} onChange={e => setFormData({...formData, examples: e.target.value})} className="w-full bg-white border border-slate-200 p-4 rounded-2xl focus:ring-4 focus:ring-indigo-100 focus:border-indigo-400 outline-none transition-all placeholder:text-slate-300" placeholder="你好吗？" />
                    </div>
                  </div>
                  <button type="submit" className="w-full py-4 mt-2 bg-gradient-to-r from-indigo-600 to-purple-600 text-white rounded-2xl font-black text-lg hover:from-indigo-700 hover:to-purple-700 transition-all shadow-lg shadow-indigo-200 hover:scale-[1.02] active:scale-95">
                    Lưu Thẻ Từ Vựng
                  </button>
                </form>
                ...
                <div className="relative flex items-center py-2">
                  ...
                  <span className="flex-shrink-0 mx-4 text-slate-400 text-xs font-black uppercase tracking-widest">Hoặc</span>
                  <div className="flex-grow border-t border-slate-200"></div>
                </div>

                <div 
                  className="group border-2 border-dashed border-indigo-200 bg-white/50 rounded-3xl p-8 text-center cursor-pointer hover:bg-indigo-50 hover:border-indigo-400 transition-all duration-300"
                ...
                >
                  ...
                  <h4 className="font-black text-lg text-slate-800">Tải lên file CSV</h4>
                  <p className="text-sm font-medium text-slate-500 mt-1">Cột bắt buộc: hanzi, pinyin, meaning</p>
